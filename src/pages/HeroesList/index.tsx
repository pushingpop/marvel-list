...
const HeroesList: React.FC = () => {
  ...
  const getData = async () => {
    ...
    fetch(`https://gateway.marvel.com/v1/public/characters?ts=${timestamp}&apikey=${PUBLIC_KEY}&hash=${hash.end()}&nameStartsWith=a&orderBy=name&offset=${20 * pageCurrent}`, {
      method: 'GET',
      headers: {
  ...
  const renderFooter = () => {
    return (
      isLoading && heroes.length < pageMax ?
      <LoadMore>
